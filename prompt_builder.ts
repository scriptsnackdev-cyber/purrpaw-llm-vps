import { SessionData, ChatMessage } from "./types.js";
...
// ---------------------------------------------------------------------------
export const buildCharacterPrompt = (session: SessionData): string => {
    ...
    if (characterPrompt.gender) prompt += `Gender: ${characterPrompt.gender}\n`;
    if (characterPrompt.age) prompt += `Age: ${characterPrompt.age}\n`;
    
    if (characterPrompt.persona) {
        prompt += `[PERSONA/PERSONALITY]:\n${characterPrompt.persona}\n`;
    } else {
        if (characterPrompt.personality) prompt += `[PERSONALITY]: ${characterPrompt.personality}\n`;
        if (characterPrompt.background) prompt += `Background: ${characterPrompt.background}\n`;
    }
    prompt += `</CHARACTER>`;
    return prompt;
};
...
// ---------------------------------------------------------------------------
export const buildCharEmoPrompt = (orderedMessages: ChatMessage[] = []): string => {
...
// ---------------------------------------------------------------------------
export const buildUserPrompt = (session: SessionData, userHeartrate: number = 75, ragContext: any = { persona: [] }, memoryNotes: any[] = []): string => {
    ...
    if (user_character?.gender) prompt += `Gender: ${user_character.gender}\n`;
    if (user_character?.age) prompt += `Age: ${user_character.age}\n`;
    if (user_character?.persona) prompt += `Persona: ${user_character.persona}\n`;
    if (relation_prompt) prompt += `[RELATIONSHIP & SETTING]: ${relation_prompt}\n`;
    prompt += `Current Heartrate: ${userHeartrate} BPM\n`;
    
    if (ragContext?.persona?.length > 0) {
        prompt += `[RECALLED_MEMORIES]:\n`;
        ragContext.persona.forEach((m: string) => prompt += `- ${m}\n`);
    }

    if (memoryNotes && memoryNotes.length > 0) {
        prompt += `[LONG_TERM_MEMORIES]:\n`;
        memoryNotes.forEach((m: any) => {
...
// ---------------------------------------------------------------------------
export const buildSummaryPrompt = (recentSummary: string = ""): string => {
    let prompt = `<SUMMARY>\n`;
    if (recentSummary) {
...
// ---------------------------------------------------------------------------
export const buildStaticPrompt = (
    session: SessionData, 
    orderedMessages: ChatMessage[] = [], 
    userHeartrate: number = 75, 
    ragContext: any = { persona: [] }, 
...
export const buildDynamicPrompt = (history: string, recentSummary: string = "") => {
...
export const buildOutputSpecifications = (): string => {
    return "";
};
...
// ---------------------------------------------------------------------------
export const formatChatHistory = (orderedMessages: ChatMessage[], userCharacterName: string, characterName: string): string => {
    const msgs = Array.isArray(orderedMessages) ? orderedMessages : [];
    ...
    return last10Messages.map((msg) => {
        let finalContent = '';
        const data = (msg as any).message_data;
...
export const constructFinalPrompt = (
    ...
    memoryNotes: any[] = [],
    userHeartrate: number = 75,
    ragContext: any = { common_sense: [], persona: [] },
    totalMessages: number = 0,
    startingChat: string = ""
): string => {
    const staticPart = buildStaticPrompt(session, orderedMessages, userHeartrate, ragContext, totalMessages, memoryNotes);
    const dynamicPart = buildDynamicPrompt(history, (session as any).recent_summary || "");
