...
export const applyStateLogic = (targetBlock, sourceBlock, limits) => {
    if (!sourceBlock)
        return targetBlock;
    ...
    Object.keys(sourceBlock).forEach(key => {
        ...
        if (isNaN(currentVal))
            currentVal = (limits.min + limits.max) / 2;
        let delta = 0;
        let isDelta = false;
        // Check format
...
export const resolveFallbackState = (currentJson, history) => {
    ...
    if (!baseState.personality_traits)
        baseState.personality_traits = {};
    ...
    if (currentJson) {
        ...
        ['scene_detail', 'mobile_updates', 'reply', 'internal_thought', 'dominant_emotion', 'heartbeat', 'summary', 'image_url'].forEach(k => {
            if (currentJson[k] !== undefined)
                resolved[k] = currentJson[k];
        });
    }
    // 4. Final Safety: Ensure resolved has the keys (in case applyStateLogic somehow lost them, though unlikely)
    if (!resolved.personality_traits)
        resolved.personality_traits = {};
    REQUIRED_TRAITS.forEach(t => {
        if (resolved.personality_traits[t] === undefined)
            resolved.personality_traits[t] = 50;
    });
