    ...
    try {
        ...
        if (recentMessages) {
            for (const msg of recentMessages) {
                ...
                if (accumulatedChars + text.length > totalCharLimit)
                    break;
        ...
        if (action === 'chat' && fromMessageId) {
            ...
            if (idx !== -1)
                orderedMessages = orderedMessages.slice(0, idx);
        }
        // Consolidate memories securely on the backend
        const consolidatedMemories = new Map();
        ...
        sessionData.recent_summary = recentSummary;
        ...
        let fullPrompt = constructFinalPrompt(sessionData, orderedMessages, historyStr, sessionData.user_character?.name || "User", messageText || "", action, memoryNotes, user_heartbeat, { common_sense: [], persona: [] }, orderedMessages.length, sessionData.character?.prompt?.starting_chat);
        // Sanitize names
