export const convertLegacyToTagsFormat = (legacyText) => {
    if (!legacyText)
        return '';
    const processInline = (text) => text.replace(/\*([^*]+)\*/g, '<h>$1</h>');
    let taggedText = '';
    let lastIndex = 0;
    const regex = /('''([\s\S]*?)''')|(\"([^\"]*)\")|(\(([^)]*)\))/g;
    let match;
    while ((match = regex.exec(legacyText)) !== null) {
        ...
        if (preMatchText.trim())
            taggedText += `<a>${processInline(preMatchText)}</a>`;
        if (match[1])
            taggedText += `<b>${processInline(match[2])}</b>`;
        else if (match[3])
            taggedText += `<q>${processInline(match[4])}</q>`;
        else if (match[5])
            taggedText += `<internal>${processInline(match[6])}</internal>`;
        lastIndex = regex.lastIndex;
    }
    const postMatchText = legacyText.substring(lastIndex);
    if (postMatchText.trim())
        taggedText += `<a>${processInline(postMatchText)}</a>`;
...
export const addLine = (prompt, label, value) => {
...
export const cleanTagsAndRemoveDisallowed = (text) => {
    if (!text)
        return '';
